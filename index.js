(function(){
	let adverbRatio = 3, passiveRatio = 5;
	let hardLevel = 10, veryHardLevel = 14;
	let minLevel = 21.43, fWord = .5, fLetter = 4.71;
	let cleanRegex = new RegExp('[^-a-z0-9.!?; ]', 'gi');
	let endSenRegex = new RegExp('[.?!:][ ]');
	let inputArea = document.getElementById("text-area"), text =
...
`;
	...
	function counters(){
		document.querySelector("#qualifier").innerHTML = `You have used ${data.qualifying} qualifier${data.qualifying > 1 ? "s" : ""}.<br><i>(Try to replace them.)</i>`;
		document.querySelector("#adverb").innerHTML = `You have used ${data.adverbs} adverb${data.adverbs > 1 ? "s" : ""}.<br><i>(Try to use ${Math.round(data.paragraphs/adverbRatio)} or less.)</i>`;
		document.querySelector("#passive").innerHTML = `You have used passive voice ${data.passiveVoice} time${data.passiveVoice > 1 ? "s" : ""}.<br><i>(Aim for ${Math.round(data.sentences/passiveRatio)} or less.)</i>`;
		document.querySelector("#complex").innerHTML = `${data.complex} phrase${data.complex > 1 ? "s" : ""} could be simplified.`;
		document.querySelector("#hardSentence").innerHTML = `${data.hardSentences} of ${data.sentences} sentence${data.sentences > 1 ? "s are" : " is"} hard to read.`;
		document.querySelector("#veryHardSentence").innerHTML = `${data.veryHardSentences} of ${data.sentences} sentence${data.sentences > 1 ? "s are" : " is"} very hard to read.`;
	}
	...
	function getDifficultSentences(p){
		...
		let hardOrNot = sentences.map(sent => {
			let cleanSentence = sent.replace(cleanRegex, "")+".";
			...
			if (words < veryHardLevel){return sent;}
			else if (level >= hardLevel && level < veryHardLevel){
				data.hardSentences += 1;
				return `<span class="hardSentence">${sent}</span>`;}
			else if (level >= veryHardLevel){
	...
	function getPassive(sent){
		...
		let words = sent.replace(cleanRegex, "").toLowerCase().split(" ");
		let ed = words.filter(word => word.match(/ed$/));
		if (ed.length > 0){
			ed.forEach(match => {originalWords = checkPrewords(words, originalWords, match);});}
	...
	function getSentenceFromParagraph(p){
		let sentences = p.split(endSenRegex).filter(s => s.length > 0).map(s => s+".");
		return sentences;}

	function calculateLevel(letters, words, sentences){
		if (words === 0 || sentences === 0){return 0;}
		let l = Math.round(fWord*words/sentences+fLetter*letters/words-minLevel);
		return l <= 0 ? 0 : l;}

	function getAdverbs(sentence){
		let lyWords = getLyWords();
		return sentence.split(" ").map(word => {
				if (word.replace(cleanRegex, "").match(/ly$/) &&
						lyWords[word.replace(cleanRegex, "").toLowerCase()] === undefined){
					data.adverbs += 1;
	...
	function findAndSpan(sentence, string, type){
		...
		if (index >= 0){
			data[a[type]] += 1;
			sentence = sentence.slice(0, index)+`<span class="${type}">` +
					sentence.slice(index, index+string.length)+"</span>" +
